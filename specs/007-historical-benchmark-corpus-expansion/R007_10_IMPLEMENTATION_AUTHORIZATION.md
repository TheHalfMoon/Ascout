...
**CI-stability repair ledger:** Issue #242 (`CI_STABILITY_REPAIR_T024_T107 = CLOSED_CANONICAL`)  
**T113 publication ledger:** Issue #208  
**Canonical authorization base:** `b8b1776c5d4e6dfddb285b916628589c408c369a`  
**Canonical authorization-base tree:** `e9212556b9ab569ef7306591367b791f205e97f3`
...
9. Issue #238 canonical R007-10 planning closeout;
10. Issue #240 authorization ledger;
11. Issue #242 / PR #243 canonical CI-stability repair;
12. immutable failed T113 r1 run `34130848099`, attempt `1`;
13. immutable failed T113 r2 run `34158105972`, attempt `1`.
...
- planning merge: `f352f038cb19c266ca05e83c85d37bc153204ef5`;
...
The planning merge no longer equals current `main` because a separately governed CI-stability repair subsequently became canonical. That forward-only repair does not expand R007-10 authority.

## Authorization attempt 1 is stale and disqualified

The first authorization candidate was PR #241 at exact head:

`a41ef6c0d71800d61e735631868f9578d83e4f92`

It is immutable failed qualification evidence and MUST NOT be salvaged or reused.

Observed attempt-1 evidence:

- PR #241: CLOSED WITHOUT MERGE;
- Self Verification run `34172827869`: SUCCESS;
- Project CI run `34172827878`: original-attempt FAILURE;
- five of six Project CI lanes succeeded;
- the only failing lane was `windows-2025 / node-24`, job `101896303598`;
- exact measured failures were unrelated local Vitest deadline failures in T024 and T107 tests;
- the authorization candidate itself changed only this authorization artifact;
- no attempt-1 CI result, review state, head, base, mergeability state, or other qualification evidence may qualify this recreated candidate.

No rerun-to-green, rebase, force-push, stale-head reuse, or retroactive qualification is permitted.

## Canonical CI-stability prerequisite

The attempt-1 Windows deadline failures were repaired prospectively under Issue #242 / PR #243.

Canonical repair proof:

- repair issue: #242 — `CI_STABILITY_REPAIR_T024_T107 = CLOSED_CANONICAL`;
- repair PR: #243 — MERGED;
- exact qualified repair head: `09b7db54bb3f4f5ff190a12779226c2e8ee7f7c5`;
- repair-head tree: `e9212556b9ab569ef7306591367b791f205e97f3`;
- repair diff: exactly two test paths, exactly one local `15_000ms` Vitest timeout binding in each, with no assertion, fixture, workflow, or product/runtime mutation;
- Self Verification run `34173967230`: SUCCESS on the exact repair head;
- Project CI run `34173967236`: original-attempt SUCCESS across all six required OS/Node lanes;
- independent exact-head CodeRabbit review run `5e8a51c7-7b60-445b-ac00-d4b8c2ca1835`: no actionable comments and no current merge-readiness risk;
- unresolved material review threads: zero;
- guarded merge used exact expected head `09b7db54bb3f4f5ff190a12779226c2e8ee7f7c5`;
- repair merge/current canonical `main`: `b8b1776c5d4e6dfddb285b916628589c408c369a`;
- repair merge tree: `e9212556b9ab569ef7306591367b791f205e97f3`;
- ordered repair merge parents: `f352f038cb19c266ca05e83c85d37bc153204ef5`, then `09b7db54bb3f4f5ff190a12779226c2e8ee7f7c5`;
- merge tree equals the qualified repair-head tree;
- GitHub repair merge verification: `verified=true`, `reason=valid`.

This repair changes test execution budgets only. It grants no R007-10 implementation authority by itself and does not alter T113 causal truth.

## Immutable T113 evidence and current truth
...
No later recovery, workflow, execution, publication, T114, security-research, or other product unit becomes authorized by implication.
...
R007-10 MUST add exactly one deterministic regression within the existing:
...
7. require malformed external JSON to be rejected by the existing report parser;
...
The implementation SHOULD use the narrowest stable rejection assertion supported by the existing test style. The existing direct malformed-report regression uses `SyntaxError`; production error classification MUST NOT be changed merely to make an assertion convenient.

No fixture mode, production behavior, or membership implementation change is authorized because the existing fixture already exposes `malformed` mode.
...
2. exact branch ancestry from authorization base `b8b1776c5d4e6dfddb285b916628589c408c369a` unless live `main` changes, in which case this candidate becomes stale and must not be merged;
...
No qualification evidence from authorization attempt 1 / PR #241 may satisfy any item in this gate.
