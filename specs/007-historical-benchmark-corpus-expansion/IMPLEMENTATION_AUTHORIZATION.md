...
**Canonical base:** `0ee5a26a6ce249dcb8d944e0fe40c823b1a4e49b`  
...
16. Issue #159 canonical CI-stability closeout
17. Issue #157 implementation-authorization ledger
...
- **Planning independent review:** CodeRabbit issue comment `5541086338`, exact planning head, no material findings
...
The planning merge does not itself authorize implementation.

## Canonical CI-stability prerequisite

The first implementation-authorization attempt, PR #158 at head `01fea18f55d27dfaae6994530605bf9018ed765d`, was not merged. Its exact-head Project CI exposed moving hosted-Windows default-5000ms test deadlines. That evidence was repaired canonically before implementation authority was recreated.

The canonical prerequisite is:

- **Repair ledger:** Issue #159, `CI_STABILITY_REPAIR = CLOSED_CANONICAL`
- **Repair PR:** #160 (`MERGED`)
- **Qualified repair head:** `11be57fced4a85d73e995c6c2e714b11287931cb`
- **Qualified repair tree:** `8a3b9f11b145808635a7e65f98a8671f479a0cba`
- **Repair Project CI:** run `33928947115`, exact-head 6/6 success
- **Repair Self Verification:** run `33928947131`, exact-head success
- **Repair independent review:** CodeRabbit issue comment `5550767054`, exact head, no material findings
- **Repair review threads:** zero unresolved threads
- **Repair merge SHA / current authorization base:** `0ee5a26a6ce249dcb8d944e0fe40c823b1a4e49b`
- **Repair merge tree:** `8a3b9f11b145808635a7e65f98a8671f479a0cba`
- **Repair merge parent 1:** `b755e8765589ebe5c8a4ee6cb9cc9381e4e4fd45`
- **Repair merge parent 2:** `11be57fced4a85d73e995c6c2e714b11287931cb`
- **Repair merge signature:** GitHub-verified PGP signature present (`verified=true`, `reason=valid`)

PR #158 is closed and stale by construction. None of its CI or review evidence qualifies this recreated authorization.

This file is recreated from canonical main `0ee5a26a6ce249dcb8d944e0fe40c823b1a4e49b`. It becomes effective only after this exact authorization head is independently qualified, guarded-merged, and post-merge verified. It does not backdate implementation authority.
...
Implementation MUST reverify the same identity/license/runtime facts and additionally prove before replay that the exact pinned Node runtime exposes the Iterator semantics required by the regression oracle. No benchmark-only polyfill, source rewrite, compatibility shim, alternate oracle, or test weakening is authorized.
...
- focused manifest/harness validation under the current repository tests where an executable route exists;
- full exact-head Project CI across Ubuntu 24.04, macOS 14, Windows Server 2025 x Node 22/24;
...
- no secrets, live network, hosted service, or undeclared mutable-state requirement during the measured oracle;
...
T113 requires exact one-path purity, focused result validation/assertions where executable, exact-head Project CI 6/6, fresh independent substantive exact-head review, zero unresolved material threads, guarded expected-head merge, ordered-parent/tree/signature/PR/main post-merge proof, and durable `T113 = CLOSED_CANONICAL` before T114.
...
4. focused proof plus full repository typecheck/test/build as applicable and executable;
...
It becomes effective only when this recreated authorization file itself is merged into canonical `main` and the authorization merge identity is post-merge verified.
...
2. the authorization PR is based on exact canonical base `0ee5a26a6ce249dcb8d944e0fe40c823b1a4e49b` with `behind_by=0` and that commit as unique merge base;
3. exact-head Project CI succeeds across all six required lanes;
4. exact-head Self Verification succeeds;
5. a fresh independent substantive exact-head review finds no unresolved material correctness, governance, evidence-integrity, security/hermeticity, portability/runtime, scope, or cross-artifact issue;
6. unresolved material review threads are zero;
7. guarded expected-head merge succeeds without intervening main movement;
8. post-merge ordered parents, merged tree, GitHub signature, PR merged state, and canonical main identity are verified;
9. Issue #157 records `SPEC_007_IMPLEMENTATION_AUTHORIZATION = CLOSED_CANONICAL / EFFECTIVE`.

Until all nine conditions are satisfied, T110 remains unauthorized.