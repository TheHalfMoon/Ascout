...
- [ ] T018 Implement `remote:<sha256(normalized-credential-free-remote)>` and `local:<sha256(canonical-real-path)>` repository identity plus HEAD/detached/shallow in `src/git.ts`; raw remote/local location material never persists.
...
- [ ] T020 Implement zero-context tracked diff + nonignored untracked changed-file scope in `src/git.ts`, preserving rename old/new path identity.
...
- [ ] T081 Add deterministic receipt/golden checks across OS path handling, proving original receipt candidate spellings are rejected before lossy normalization and never repaired, including canonical slash-separated persisted relative paths plus rejection of absolute/drive/UNC/URI/backslash/dot-segment/duplicate-separator/trailing-separator forms, with explicit `src//file.ts` and `src/` regression cases.
