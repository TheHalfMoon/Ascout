...
- [ ] T009 [P] Receipt v1 contract + semantic-invariant tests: config/receipt task identifier parity; privacy-safe `remote:<sha256>`/`local:<sha256>` source IDs; full Git object IDs only for `source.start.head_sha` and `comparison.base_ref` (40-hex SHA-1 or 64-hex SHA-256), with semantic equality required and malformed/abbreviated/symbolic/mismatched values rejected; every `changed_new_line_ranges` pair must satisfy `start <= end`, with `[10, 1]` rejected before any coverage/exercise arithmetic; canonical relative-path validation for every persisted path-bearing field, proving the **original receipt candidate spelling is rejected before any lossy normalization/collapse/resolution** (reject POSIX absolute, Windows drive/UNC, URI-absolute, backslash-canonicalization violations, `.`/`..` traversal, duplicate separators such as `src//file.ts`, and trailing separators such as `src/`, and prove none is repaired into a valid-looking path); root `evidence[]`; unique/resolvable run/task/artifact evidence references; fixed task/status/selection shapes; rename requires `previous_path`; `NOT_RUN`/`BLOCKED`/`ERROR` require non-empty reasons; exercise state/count/reason invariants; source stability/completeness; admission invariants; aggregate counts and exit precedence.
...
- [ ] T011 [P] Git diff tests: add/modify/delete/rename/type/binary/untracked line semantics, rename old/new path fidelity, full resolved HEAD object identity, and generated changed new-line ranges with positive `start <= end` only.
...
- [ ] T018 Implement `remote:<sha256(normalized-credential-free-remote)>` and `local:<sha256(canonical-real-path)>` repository identity plus full resolved HEAD object ID/detached/shallow in `src/git.ts`; reject unborn HEAD for M1 `check`; raw remote/local location material never persists.
...
- [ ] T020 Implement zero-context tracked diff + nonignored untracked changed-file scope in `src/git.ts`, preserving rename old/new path identity, emitting only valid `start <= end` changed new-line ranges, and recording the exact resolved comparison base object ID equal to source-start HEAD.
...
- [ ] T025 Implement receipt model/exit decision plus one pure semantic validator in `src/receipt/model.ts`: require full Git object IDs and enforce `comparison.base_ref == source.start.head_sha` for `working_tree_vs_head`; reject any changed new-line range with `start > end` before changed-line/coverage/exercise arithmetic; construct repository/run-relative receipt candidates from transient host/tool paths, then validate each candidate's **original spelling before any lossy normalization, separator collapse, trailing-separator removal, or dot-segment resolution**; reject rather than repair all noncanonical absolute/drive/UNC/URI/backslash/dot-segment/duplicate-separator/trailing-separator forms; only after raw-form rejection perform namespace-containment/canonicality checks; also enforce root evidence collection, unique/resolvable evidence/task/artifact links, run-bound evidence, weak fingerprints, admission state, strict task/selection/rename/exercise/reason invariants, aggregate counts, separate stability/completeness, and exit precedence.
- [ ] T026 Implement JSON renderer; require JSON Schema validation and semantic receipt validation before emission, including exact source/comparison binding, changed-line range validity, and path-namespace containment. Any internal/future receipt acceptance path must reuse the same semantic validator.
...
- [ ] T028 [P] Command-provenance/admission integration matrix proving effective changed package/Ascout/TypeScript/ESLint/Vitest/Jest/**pytestBasic** authority paths cause `NOT_RUN(command_surface_changed)` before process launch/load; include applicable pytest config fixtures (`pytest.ini`, `pyproject.toml`, `setup.cfg`, or `tox.ini`) when used by the invocation.
...
- [ ] T033 End-to-end check receipt test: privacy-safe source ID, exact full HEAD object identity with `comparison.base_ref == source.start.head_sha`, only valid `start <= end` changed new-line ranges, canonical relative persisted paths, admission, comparison, config digest, tasks, resolvable evidence/artifacts, stability/completeness; reject malformed/abbreviated/symbolic/mismatched source/base identities and inverted `[10, 1]` range; semantic validation rejects the **original** absolute/drive/UNC/URI/backslash/dot-segment/duplicate-separator/trailing-separator candidate before lossy normalization can erase it, including `src//file.ts` and `src/`, proves invalid spellings are never repaired, rejects dangling/cross-run/cross-task references, and proves incomplete cannot green.
...
- [ ] T037 Implement basic pytest task in `src/tools/pytest.ts`; no Python affected/environment/coverage architecture; report effective pytest config source when used.
- [ ] T038 Implement effective command-surface classification and changed-path intersection in `src/discovery.ts` for package scripts, Ascout overrides, loaded compiler/lint/Vitest/Jest configs, and effective pytestBasic config.
...
- [ ] T041 Wire `ascout check` + `--allow-changed-command-surface`; never persist the flag as trust and never green over admission-refused work or unresolved exercise/evidence proof.
...
- [ ] T066 [P] Agent receipt <=16 KiB tests preserving identity/status/admission/gaps/completeness + resolvable evidence refs + omitted totals.
- [ ] T067 Cross-format consistency contract: terminal/JSON/agent derive from one semantically validated model.
...
- [ ] T077 Add absolute assertions: cross-tree evidence leakage=0; binding-integrity violations=0; stable runs with remaining material `NOT_EXERCISED`/`UNRESOLVED` gaps returning exit 0=0 (must map to exit 4 absent higher precedence).
...
- [ ] T081 Add deterministic receipt/golden checks across OS path handling and Git object formats: 40-hex SHA-1 and 64-hex SHA-256 HEAD/base identity shapes, exact `base_ref == source.start.head_sha`, inverted changed-line range rejection, and original receipt candidate spellings rejected before lossy normalization and never repaired, including canonical slash-separated persisted relative paths plus rejection of absolute/drive/UNC/URI/backslash/dot-segment/duplicate-separator/trailing-separator forms, with explicit `src//file.ts` and `src/` regression cases.
...
- [ ] T088 Run quickstart + all contract/semantic-validation/integration/benchmark gates from clean checkout; record release-candidate evidence without publishing.
...
1. Inert CLI.
2. Source/config/process/receipt/admission/evidence primitives.
3. One source-bound task receipt with resolvable evidence and changed-command default refusal.
...
Return to planning if implementation requires a second product runtime dependency, DB/daemon, generic plugin SDK, semantic graph, arbitrary config workflow, automatic untrusted execution, shell-string commands, recursive widening, persistent changed-surface trust grant, automatic agent admission escalation, a separate receipt-validation interpretation, or an exit/report rule hiding task/exercise/admission/evidence gaps behind success.