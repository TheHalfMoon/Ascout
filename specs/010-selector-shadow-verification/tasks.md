...
6. execute one bounded full-suite structured JSON reference with no shell, retry, or implicit install;
...
1. retain all existing same-repository eligibility, permissions, exact-head, install/build, and self-verification steps unchanged except the bounded integration needed here;
2. invoke canonical `benchmarks/selector-shadow.mjs` immediately after successful Spec 006 capture using the exact just-produced receipt/envelope;
3. write `selector-shadow-observation.json` under the same runner-temp self-verification evidence directory;
4. add exactly that file to the existing pinned artifact upload path list;
5. retain existing artifact name, retention, least privilege, and exact action pins unless implementation-time revalidation requires a separately reviewed amendment;
6. do not fail because selector misses exist;
7. fail/stop only on workflow/harness integrity conditions defined by the plan.

### Required live evidence

The exact final eligible same-repository T118 PR head must produce one downloadable selector-shadow observation bound to the exact PR head receipt/envelope. Static YAML review is insufficient.

If the observation is unavailable for a reason outside the planned exhaustive boundary, do not patch around it. Record the reason and return to planning.

### Hard prohibitions

No new workflow/action/dependency/permission/secret/fork execution; no `pull_request_target`; no selector/product/receipt/schema/Project-CI/historical-result/release mutation.

### Qualification gate

All T117-style exact-head gates apply, plus:

- live final-head selector-shadow artifact exists;
