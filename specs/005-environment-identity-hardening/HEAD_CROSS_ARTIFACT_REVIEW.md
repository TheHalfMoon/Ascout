...
- [x] F8 closed: pre-open contained target identity → one open → immediate descriptor identity match before bytes → descriptor-only hash → pre/post descriptor stability → post-read contained path/object identity recheck.
- [x] Path/size/timestamps alone cannot substitute for object identity; no reopen-and-hash fallback.
- [x] Object-binding must pass Ubuntu/macOS/Windows × Node 22/24 or T105 is `NO_GO`.
- [x] No generalized filesystem sandbox/watch/security abstraction.
- [x] No process/install authority; privacy/bounded-memory rules explicit.
...
- [x] F8 containment-to-open lockfile race lacked file-object binding.
...
`INTERNAL_CONSISTENCY = PASS_AFTER_F1_F2_F3_F4_F5_F6_F7_F8_RECONCILIATION`
