...
6. When discovery resolved package-manager authority from root `package.json`, package-manager version MUST be recovered from that same already-authoritative exact declaration after confirming that it names the same resolved manager. Because discovery only resolves declaration-led authority after validating exact `manager@x.y.z`, missing/unreadable/malformed/re-mismatched authoritative declaration state is an integrity failure, not an optional `null` version. When discovery resolved authority from a recognized lockfile, version remains `null`.
...
12. Semantic validation rejects inconsistent nullable groups: `package_json` requires manager + non-null exact version; `lockfile` requires manager + null version; `unavailable` requires null manager/version; lockfile path/digest must appear together and obey manager/source consistency.
13. Updated same-source/build semantic and JSON Schema validators must accept canonical older v1 receipts without `environment`.
14. New environment-bearing receipts must be accepted by the same-source/build semantic and JSON Schema validators. The exact prior strict schema is expected to reject them and that version-skew behavior must be tested/documented rather than mislabeled as forward compatibility.
15. All repository-supported receipt validators/consumers from the producing canonical source/build revision must move in lockstep with this additive v1 extension; no same-build consumer may pin a stale schema copy. `run.ascout_version` is only a product-version label and is not an exact source/schema-revision key.
...
- package-json-derived manager always carries the exact validated non-null version from the same authoritative declaration, and any inability to reproduce that declaration state fails integrity;
