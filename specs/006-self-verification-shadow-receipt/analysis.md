...
## 6. YAGNI consistency — F5

Both YAGNI reviews explicitly retain canonical composer reuse, six-field source binding, and the prohibition on duplicate source-state/digest machinery.

**Result:** F5 RECONCILED.

## 7. Exit-2 disposition — F6

A prior ambiguity remained because the planning artifacts identified exits `0/1/3/4` as successful shadow captures but did not exhaustively define the case where Ascout exits `2` while stdout nevertheless contains an otherwise-valid receipt.

The final rule is explicit and exhaustive:

1. parse receipt and run current schema + semantic validation;
2. require process exit equals receipt summary exit;
3. require the six-field source binding to S;
4. if the agreed exit is `2`, fail the harness **even if every preceding receipt check succeeded**;
5. perform that exit-2 rejection before receipt SHA-256, envelope creation, or artifact upload;
6. only exits `0`, `1`, `3`, and `4` may become retained `SHADOW_NON_GATING` evidence.

T107 focused proof must construct an otherwise-valid/current-schema-valid/semantically-valid/process-consistent/source-bound exit-2 receipt and prove the harness rejects it before digest/envelope/upload.

This preserves exit `2` as an execution/config/integrity failure class rather than silently promoting it into durable self-verification evidence.

**Result:** F6 RECONCILED / EXIT CLASSIFICATION EXHAUSTIVE.

## 8. Result honesty / authority

No automatic changed-command admission. Only a schema-valid, semantically valid, process-exit-consistent, source-bound receipt whose exit is in `0/1/3/4` may become successful capture. Valid exits 0/1/3/4 remain exact shadow truth; workflow green means capture integrity only. Exit 2 is never successful shadow evidence.

**Result:** ALIGNED WITH NO-GREEN-BY-OMISSION.

## 9. Envelope / privacy

Envelope remains external qualification metadata binding H/HT, B, M, target H/HT, receipt exit/digest/filename, `SHADOW_NON_GATING`. S remains in-memory integrity evidence and is not a new schema surface. Exit-2 receipts never reach envelope generation. Raw repo/path/user/host/env/secret data is forbidden.
...
T107 must prove merge-base cases, exact reconstruction, test-before-build adapters, canonical S capture, equality success, **each of the six independent source-field mismatch failures**, valid exits 0/1/3/4, the otherwise-valid exit-2 rejection case, malformed/invalid outcomes, digest/privacy/no-admission.

T108 must prove same-repo eligible live artifact, exclusion of fork execution before checkout, actual exact-H built `composeSourceState` + validators, six-field source binding, exit restricted to 0/1/3/4 for retained artifacts, and bounded artifact retention.
...
`CROSS_ARTIFACT_ANALYSIS = PASS_AFTER_F1_F2_F3_F4_F5_F6_RECONCILIATION`
...
5. both YAGNI reviews must preserve the F4 binding and no-duplicate-algorithm rule;
6. exit 2 is always harness failure, including the otherwise-valid receipt case.

Planning still does not authorize implementation. All earlier-head CI/review evidence is stale after F6 reconciliation.