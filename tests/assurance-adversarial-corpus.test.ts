...
function target(targetId = "target:ua-p01-t16") {
  return createAssuranceTargetV1({
    target_id: targetId,
    source_start_identity: source(),
    base_revision: BASE,
    change_set_identity: "changeset:ua-p01-t16",
...
function runInput(runId = "run:ua-p01-t16:1") {
  return {
    run_id: runId,
    plan_id: "plan:ua-p01-t16",
    target_id: "target:ua-p01-t16",
    engine_id: "engine:ascout-native",
    qualification_id: "qualification:ascout-native-v1",
    configuration_identity: configuration(),
    command_identity: {
    ...
    input_context_manifest: {
      manifest_id: "manifest:ua-p01-t16",
    ...
    finding_refs: ["finding:ua-p01-t16"],
    coverage_refs: [],
...
function evidenceInput(
  evidenceId = "evidence:ua-p01-t16:primary",
...
function finding() {
  return createFindingV1({
    finding_id: "finding:ua-p01-t16",
    kind: "CORRECTNESS",
    severity: "HIGH",
    confidence_class: "HIGH",
    validation_state: "VALIDATED",
    rule_check_identity: "check:ua-p01-t16",
    primary_location: {
      path: "src/example.ts",
      line: 17,
      column: 4,
    ...
    producer_observations: [
      {
        observation_id: "observation:ua-p01-t16",
        producer_id: "producer:ascout-native",
        run_id: "run:ua-p01-t16:1",
        target_id: "target:ua-p01-t16",
        observation_state: "CONCERN",
        evidence_refs: ["evidence:ua-p01-t16:primary"],
        observed_at_epoch_ms: 2_300,
      },
    ],
    reproduction_proof_refs: ["evidence:ua-p01-t16:primary"],
    first_seen_target_id: "target:ua-p01-t16",
    last_verified_target_id: "target:ua-p01-t16",
    status: "OPEN",
...
function lifecycleEvents() {
  ...
  return values.map(([sequence, previous, next, occurredAt]) =>
    createFindingLifecycleEventV1({
      lifecycle_event_id: "lifecycle:ua-p01-t16:" + sequence,
      finding_id: "finding:ua-p01-t16",
...
function supportedAssessment() {
  return createClaimAssessmentV1({
    claim_assessment_id: "assessment:ua-p01-t16",
    target_id: "target:ua-p01-t16",
    intent_id: "intent:ua-p01-t16",
    requested_claim: "RELEASE_READY",
    state: "SUPPORTED",
    ...
    omission_refs: [],
    contradiction_refs: [],
    supporting_evidence_refs: ["evidence:ua-p01-t16:primary"],
...
function graph() {
  return {
    ...
    mandatory_evidence_ids: ["evidence:ua-p01-t16:primary"],
  };
}

describe("UA-P01-T16 bounded contract boundary corpus", () => {
  it("accepts the owned valid control graph", () => {
    expect(() => validateAssuranceSemanticGraphV1(graph())).not.toThrow();
  });

  it("rejects malformed semantic structures at the exact graph boundary", () => {
    expect(() =>
  ...
  it("rejects cross-target evidence", () => {
    const wrongTargetEvidence = createEvidenceRefV1({
      ...evidenceInput(),
    ...
    expect(() =>
      validateAssuranceSemanticGraphV1({
        ...graph(),
        evidence_refs: [wrongTargetEvidence],
      }),
    ).toThrow("evidence target_id does not match AssuranceTarget");
  });

  it("rejects a dangling evidence run reference", () => {
    const danglingRunEvidence = createEvidenceRefV1({
      ...evidenceInput(),
      run_id: "run:missing",
    });
    ...
    expect(() =>
      validateAssuranceSemanticGraphV1({
        ...graph(),
        evidence_refs: [danglingRunEvidence],
      }),
    ).toThrow("evidence_refs.run_id contains dangling ref: run:missing");
  });

  it("rejects stale evidence that is silently classified as supporting", () => {
    const stale = evidence("evidence:ua-p01-t16:primary", 2_500);
  ...
  it("rejects an invalid Finding lifecycle transition", () => {
  ...
  it("rejects omitted mandatory evidence for a supported claim", () => {
    expect(() =>
      validateAssuranceSemanticGraphV1({
        ...graph(),
  ...
  it("rejects contradictory evidence inside a supported claim", () => {
    expect(() =>
      validateAssuranceSemanticGraphV1({
        ...graph(),
        claim_assessment: {
          ...supportedAssessment(),
          contradicting_evidence_refs: ["evidence:ua-p01-t16:primary"],
        },
      }),
    ).toThrow(
      "SUPPORTED claim assessment cannot contain contradicting_evidence_refs",
    );
  });

  it("rejects an unknown claim assessment state", () => {
    expect(() =>
      validateAssuranceSemanticGraphV1({
        ...graph(),
        claim_assessment: {
  ...
  it("resists object insertion-order serialization instability", () => {
    const first = {
      z: [3, 2, 1],
      nested: {
        b: 2,
  ...
  it("preserves array order as semantic rather than sorting it", () => {
