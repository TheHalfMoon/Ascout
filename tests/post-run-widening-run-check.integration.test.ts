...
function initializeFixture(): string {
  const root = mkdtempSync(join(tmpdir(), "ascout-t054-check-"));
...
fs.writeFileSync(outputPath, JSON.stringify({ testResults: [{ name: related ? "narrow.test.js" : "full.test.js" }] }));
...
`,
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: "t054-vitest-check-fixture",
      private: true,
      type: "module",
  ...
  run(root, "git", ["init", "-q"]);
  run(root, "git", ["config", "user.name", "Ascout T054 Fixture"]);
  run(root, "git", ["config", "user.email", "t054@example.invalid"]);
  run(root, "git", ["add", "."]);
...
describe("T054 runCheck bounded post-run widening", () => {
  it("runs exactly one full second pass when narrowed LCOV misses a changed production source", async () => {
    ...
    try {
      ...
      expect(task?.argv).not.toContain("related");
      ...
      const firstCoverage = receipt.artifacts.find(({ artifact_id }) => artifact_id === "test.lcov");
      const secondCoverage = receipt.artifacts.find(({ artifact_id }) => artifact_id === "test.pass-2.lcov");
      expect(firstCoverage?.relative_run_path).toBe("raw/test/coverage/lcov.info");
      expect(secondCoverage?.relative_run_path).toBe("raw/test/pass-2/coverage/lcov.info");
      expect(firstCoverage && existsSync(join(root, ".ascout", "runs", receipt.run.run_id, firstCoverage.relative_run_path))).toBe(true);
