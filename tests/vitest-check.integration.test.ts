...
type ControlledLcovMode = "absent" | "resolved_gap" | "invalid";
...
function initializeControlledBranchFixture(mode: ControlledLcovMode): string {
  const root = mkdtempSync(join(tmpdir(), `ascout-t103-${mode}-`));
  mkdirSync(join(root, "src"), { recursive: true });
...
describe("T103 runCheck branch receipt integration", () => {
  ...
  it("fails closed when branch LCOV is malformed instead of manufacturing zero branch evidence", async () => {
    const root = initializeControlledBranchFixture("invalid");
    try {
      const { receipt } = await runCheck(root);
