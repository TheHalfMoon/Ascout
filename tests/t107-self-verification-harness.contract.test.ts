...
describe("T107 exact-tree self-verification harness", () => {
  ...
  it.skipIf(process.platform === "win32")("rolls back final pathname replacement after child RELEASE checks before rollback finalization", async () => {
    const simple = createSimpleRepository();
    const outputDir = temporaryOutputPath("ascout-t107-post-release-race-");
    const repositoryTarget = join(simple.root, "post-release-target");
    const displaced = join(dirname(outputDir), "displaced-post-release-bundle");
    mkdirSync(repositoryTarget);
    let replaced = false;
    let returnedReceiptPath: string | null = null;
