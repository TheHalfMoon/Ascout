...
assert m['manifest_revision']==4 and len(m['cases'])==6
...
root=Path(tempfile.mkdtemp(prefix='t073-exact-19840207-'))
...
try:
 for c in m['cases']:
  ...
  expected_revision=3 if cid=='tanstack-streamed-query-reducer' else 2
  assert c['case_revision']==expected_revision and c['case_class']=='selection' and c['lifecycle_state']=='CASE_REVIEWED'
  ...
  if cid=='tanstack-streamed-query-reducer':
   ...
   notes=c['provenance']['review_notes']
   assert 'remains naturally present in the derived baseline' not in notes
   assert 'historical changeset file is deleted from the derived baseline because it is absent in the base tree' in notes
   assert 'explicitly excluded by reconstruction.ancillary_review' in notes
  else:
   assert ancillary==set() and keep==set() and drop==set()
  ...
  if cid=='react-hook-form-value-as-date':
   ...
   assert pkg.get('packageManager')==f'pnpm@{pm}'
  if cid.startswith('zod-'):
   assert pkg['scripts']['test']=='vitest run' and 'pnpm exec vitest related packages/zod/src/v4/core/util.ts --run' in cmd
  if cid=='tanstack-streamed-query-reducer':
 ...
 print('T073 exact-head donor/schema/synthetic/ancillary/command/conflict evidence PASS')
finally:
